...
<div class="card" style="width:100%">
    <header class="card-header">
        <p class="card-header-title">Mes Playlists</p>
           <div class="select">
            <select onchange="window.location.href = this.value">
                <option value="{{ route('playlist.index') }}">Tous les artistes</option>
                @foreach($artiste as $artistes)
                    <option value="{{ route('playlist.artiste', $artistes->nom) }}"
                        {{ $nom = $artistes->nom ? 'selected' : '' }}>{{ $artistes->nom }}</option>
                @endforeach
            </select>
        </div>
        <div class="select">
            <select onchange="window.location.href = this.value">
                <option value="{{ route('playlist.index') }}" @unless($titremus) selected @endunless>Toute les musiques</option>
        ...
        <a class="button is-info" href="{{ route('artiste.showAll') }}">Créer un artiste</a>
    </header>
